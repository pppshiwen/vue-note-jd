...
module.exports = () => {
    ...
    */
    ...
    route.post('/reg', (req, res) => {
        ...
        let regPic = mObj.regPic;
        regPasswd = common.md5(regPasswd + common.MD5_SUFFXIE);
        const insUserInfo = `INSERT INTO user(user_name,login_password,user_number,user_photo) VALUES('${regName}','${regPasswd}','${regName}','${regPic}')`;
    ...
    route.post('/login', (req, res) => {
        ...
        let password = common.md5(mObj.loginPawd + common.MD5_SUFFXIE);//111 111
    ...
    route.get('/userinfo', (req, res) => {
        ...
        const getU = `SELECT user_name,user_number,user_photo FROM user where user_id='${uId}'`;
    ...
    //笔记模块加载
    route.get('/note', (req, res) => {

        const getNote ='SELECT note_id,note_title,note_detail,note_attenction,note_pic_url,note_like,user_name,user_photo FROM note,user where note.user_id = user.user_id';
        // console.log("1");
        // res.end('1')
        db.query(getNote, (err, data) => {
            if (err) {
                console.log(err);
                res.status(500).send('database err').end();
            } else {
    ...
    //笔记具体内容加载
    route.get('/notedetail', (req, res) => {
        let noteId = req.query.noteId//获得url后面的id值
        const getNotedetail = `SELECT note_id,note_title,note_detail,note_attenction,note_pic_url,note_like,user_name,user_photo,note_create FROM note,user where note.user_id = user.user_id and note_id='${noteId}'`;
        // console.log(noteId);
        //console.log(req.query);
        // res.end('1')
        db.query(getNotedetail, (err, data) => {
            if (err) {
                console.log(err);
                res.status(500).send('database err').end();
            } else {
    ...
    route.post('/noteadd',(req,res)=> {
        ...
        form.parse(req, function(err, fields, files) {
            ...
            var note_title = fields.note_title[0];
            var note_cont = fields.note_cont[0];
            var note_like = 0;
            var user_id =36;
            // //图片地址
            var pic = files.note_pic[0].path;
           // console.log(pic);//upload\d42SebzgKhV7peqs9c5YCjRz.jpg
            note_pic = pic.replace(/\\/,'/');
            //console.log(note_pic);
            var myDate = new Date();
            var note_create = myDate.getFullYear()+'-'+(myDate.getMonth()+1)+'-'+(myDate.getDate()+1);
            const insNoteInfo = `INSERT INTO note(note_title,note_detail,note_pic_url,note_like,note_create,user_id) VALUES('${note_title}','${note_cont}','${note_pic}','${note_like}','${note_create}','${user_id}')`;
            delNoteadd(insNoteInfo, res);
        });

        function delNoteadd(insNoteInfo, res) {
            db.query(insNoteInfo, (err) => {
                if (err) {
                    console.error(err);
                    res.send({ 'msg': '服务器出错', 'status': 0 }).end();
                } else {
                    res.send("<script>alert('上传成功');location.href='http://localhost:8086/note'</script>").end();
                }
            })
        };
    });
